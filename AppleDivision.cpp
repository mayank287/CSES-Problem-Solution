...
ll minans(vector<ll> x, ll currsum, ll totalsum, ll i)
{
    if(i == 0)
    {
    ...
    return min(minans(x,currsum + x[i],totalsum,i - 1),minans(x,currsum,totalsum,i - 1));
}
...
void Mayank() 
{
 ...
 cout << minans(v,0,totalsum,n - 1) << nline;
